...
function Get-GpoPrecedenceOrder
{
...
<#
	...
	.PARAMETER OutputConfigurationScript
		This determines whether or not to output a ConfigurationScript in addition to the localhost.mof
...
function ConvertFrom-GPO
{
	...
	if ($PSCmdlet.ParameterSetName -eq "OrderedGPO")
	{
		...
		foreach ($item in $orderedgpo.gpodata)
		{
	...
	{
		$polFiles = $GPOArray | ForEach-Object{
			Write-Verbose "Gathering POL files from $($_)"
			Get-ChildItem -Path $_ -Filter registry.pol -Recurse
		} | Sort-Object -Property fullname -Unique
		
		$AuditCSVs = $GPOArray | ForEach-Object{
			Write-Verbose "Gathering AuditCSVs from $($_)"
			Get-ChildItem -Path $_ -Filter Audit.csv -Recurse
		} | Sort-Object -Property fullname -Unique
		
		$GPTemplateINFs = $GPOArray | ForEach-Object{
			Write-Verbose "Gathering TemplateINFs from $($_)"
			Get-ChildItem -Path $_ -Filter GptTmpl.inf -Recurse
		} | Sort-Object -Property fullname -Unique
		
		$PreferencesDirectory = $GPOArray | ForEach-Object{
			Write-Verbose "Gathering PrefernecesDirectory from $($_)"
			Get-ChildItem -Path $_ -Directory -Filter "Preferences" -Recurse
		} | Sort-Object -Property fullname -Unique
		
		if ($PreferencesDirectory -ne $null)
		{
			$PreferencesXMLs = $PreferencesDirectory | ForEach-Object{
				Write-Verbose "Getting PreferencesXMLs from $($_)"
				Get-ChildItem -Path $_.FullName -Filter *.xml -Recurse
			} | Sort-Object -Property fullname -Unique
		}
	}
